...
public class GameController {
    ...
    @MessageMapping("/selectColor")
    public void selectColor(@Payload ColorSelectionCom colorSelectionCom) {
        Game game = games.get(colorSelectionCom.getGameCode());
        if (game != null) {
            Player player = game.getPlayers().stream()
                    .filter(p -> p.getId() == colorSelectionCom.getPlayerId())
                    .findFirst()
                    .orElse(null);
            if (player != null) {
                player.setChosenColor(colorSelectionCom.getColor());
                messagingTemplate.convertAndSendToUser(player.getUsername(), "/queue/colorSelected", player);
            }
        }
    }
